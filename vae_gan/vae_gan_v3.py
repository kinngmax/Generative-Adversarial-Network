...
class Encoder(nn.Module):
    ...
    def __init__(self, input_channels = 1, output_channels = 1024*2, representation_size = 64):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(representation_size, representation_size, 5, 1, 2),
            nn.BatchNorm2d(representation_size),
            nn.ReLU(),

            # nc x 64*2 x 64*2
            nn.Conv2d(representation_size, representation_size, 5, stride=2, padding=2),
            nn.BatchNorm2d(representation_size),
            nn.Conv2d(representation_size, representation_size, 5, 1, 2),
            nn.BatchNorm2d(representation_size),
            nn.ReLU(),
            ...
            nn.Conv2d(representation_size*2, representation_size*2, 5, 1, 2),
            nn.BatchNorm2d(representation_size * 2),
            nn.ReLU(),
            ...
            nn.Conv2d(representation_size*4, representation_size*4, 5, 1, 2),
            nn.BatchNorm2d(representation_size * 4),
            nn.ReLU())
...
class Decoder(nn.Module):
    ...
    def __init__(self, input_size = 1024*2, representation_size = (256, 8, 8)):
        ...
        self.act1 = nn.Sequential(nn.BatchNorm2d(256), nn.Conv2d(256, 256, 5, 1, 2), nn.BatchNorm2d(256), nn.ReLU())
            # 256 x 16 x 16
        self.deconv2 = nn.ConvTranspose2d(256, 128, 5, stride=2, padding=2)
        self.act2 = nn.Sequential(nn.BatchNorm2d(128), nn.Conv2d(128, 128, 5, 1, 2), nn.BatchNorm2d(128), nn.ReLU())
            # 128 x 32 x 32
        self.deconv3 = nn.ConvTranspose2d(128, 64, 5, stride=2, padding=2)
        self.act3 = nn.Sequential(nn.BatchNorm2d(64), nn.Conv2d(64, 64, 5, 1, 2), nn.BatchNorm2d(64), nn.ReLU())
            # 32 x 64 x 64
        self.deconv4 = nn.ConvTranspose2d(64, 32, 5, stride=2, padding=2)
        self.act4 = nn.Sequential(nn.BatchNorm2d(32), nn.Conv2d(32, 32, 5, 1, 2), nn.BatchNorm2d(32), nn.ReLU())
            # 32 x 64*2 x 64*2
        self.deconv5 = nn.ConvTranspose2d(32, 1, 5, stride=2, padding=2)
    ...
    def forward(self, code):
        ...
        output = self.deconv4(output, output_size=(bs, 32, 64*2, 64*2))
        output = self.act4(output)
        output = self.deconv5(output, output_size=(bs, 32, 64*4, 64*4))
...
class VAE_GAN_Generator(nn.Module):
    ...
    def __init__(self, input_channels = 1, hidden_size = 1024*2, representation_size=(256, 8, 8)):
...
def visual(i, G, inp, recon, fake):
    ...
    for j in range(4):
        ...
        ax[j, 0].imshow(inp[j, 0], cmap = 'gray')
        ax[j, 0].axis('off')
        ax[j, 1].imshow(recon[j, 0], cmap = 'gray')
        ax[j, 1].axis('off')
        ax[j, 2].imshow(fake[j, 0], cmap = 'gray')
        ax[j, 2].axis('off')
    ...
    os.mkdir('/data_l77/shaikh/thesis/testing/gen/%d' %i)

    gen = []
    for k in range(20):
        
        noise = Variable(torch.randn(4, 1024*2)).cuda()
        rec_noise = G.decoder(noise)
        gen.append(rec_noise.detach().cpu().numpy())

    gen_im = np.array(gen[0])
    for k in range(1, len(gen)):
        gen_im = np.append(gen_im, gen[k], axis = 0)

    for k in range(len(gen_im)):
        plt.imsave('/data_l77/shaikh/thesis/testing/gen/'+ str(i) + '/' + str(k) +'.png', gen_im[k, 0], cmap = 'gray')
...
for i in range(epochs):
    ...
    for j in range(int(len(train)/4)):
        ...
        l_real = criterion(out, real.squeeze())
        out = D(rec_enc)
        l_recon = criterion(out, real.squeeze())
        out = D(rec_noise)
        l_fake = criterion(out, real.squeeze())
        disc_loss = l_real + l_recon + l_fake
    ...
    if i % 50 == 0:
        ...
        visual(i, G, inp, rec_enc, rec_noise)
